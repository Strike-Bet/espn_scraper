    ...
    for player in parsed_players:
        ...
        try:
            response = requests.post(
                url=f"{BACKEND_URL}/api/betting-events/by-player",
                headers=headers,
        ...
        for event_id in event_ids:
            event_resp = requests.get(f"{BACKEND_URL}/api/betting-events/{event_id}", headers=headers)
            ...
            if closed:
                close_resp = requests.post(
                    f"{BACKEND_URL}/api/betting-events/{event_id}/complete",
                    headers=headers,
                ...
                update_resp = requests.put(
                    f"{BACKEND_URL}/api/betting-events/{event_id}",
                    headers=headers,
                    json={"result": str(result)}
                )
